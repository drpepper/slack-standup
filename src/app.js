...
//
//   client.chat.postMessage / client.chat.update
//     Posts as the bot user — shows the configured bot avatar and display name.
//     Used for the main standup messages visible to everyone in the channel.
//
//   client.chat.postEphemeral
//     Posts as the bot user (same avatar/name), but only visible to one user.
//     Used for feedback, errors, and status messages once the bot is confirmed
//     to be in the channel.
//
//   respond()
//     Posts via the slash command response URL generated by Slack. Appears with
//     the app identity (different name, no avatar). Used only when the bot is not
//     confirmed to be in the channel — respond() is the only mechanism that works
//     without channel membership. Specifically:
//       • "Fetching active channel members…" (sent before membership is verified)
//       • "not_in_channel" errors (bot cannot join the channel)
//       • catch-all error handler (fallback when postEphemeral itself may fail)
//
...
app.command('/standup', async ({ command, ack, respond, client, say }) => {
  ...
  if (lower === 'next') {
    ...
    if (!sess) {
      await client.chat.postEphemeral({ channel: channelId, user: userId, text: errorText('No standup in progress. Start one with `/standup`.') });
      return;
    }
  ...
  if (lower === 'status') {
    const sess = session.get(channelId);
    if (!sess) {
      await client.chat.postEphemeral({ channel: channelId, user: userId, text: errorText('No standup in progress.') });
      return;
    }
  ...
  if (lower.startsWith('add ')) {
    ...
    if (uids.length === 0) {
      await client.chat.postEphemeral({ channel: channelId, user: userId, text: errorText('Please specify a user, e.g. `/standup add @alice` or `/standup add j`.') });
      return;
    }
  ...
  if (lower.startsWith('remove ')) {
    ...
    if (uids.length === 0) {
      await client.chat.postEphemeral({ channel: channelId, user: userId, text: errorText('Please specify a user, e.g. `/standup remove @alice` or `/standup remove j`.') });
      return;
    }
  ...
  if (userIds.length === 0) {
    ...
    await respond({ text: ':hourglass: Fetching active channel members…', response_type: 'ephemeral' }); // respond(): sent before channel membership is confirmed
    let members;
    try {
    ...
    if (userIds.length === 0) {
      await client.chat.postEphemeral({ channel: channelId, user: userId, text: errorText('No active members found in this channel.') });
      return;
    }
  }
  ...
  messageTs.set(channelId, msg.ts);
    ...
    await respond({ text: errorText(`Something went wrong: ${err.message}`), response_type: 'ephemeral' }).catch(() => {}); // respond(): last resort, postEphemeral may itself fail
  }
});
